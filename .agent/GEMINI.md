...
## 🛠️ CORE CONFIGURATION (Read First)
...
| **Skill Registry**| `.agent/registry.json` | Skill discovery & triggers |
...
```
Intent Analysis → Consult .agent/registry.json
    │
    ├── Find Matching Skill (Core, Tech, Process, custom)
    ├── Discovery: Search metadata for domain match
    └── Activation: Load SKILL.md from relevant subdirectory
```

### 2. Implementation Lifecycle
1. **Activation**: Read `SKILL.md` (INDEX only) for relevant sections.
2. **Execution**: Specialised agent performs task with injected knowledge.
3. **Review**: Invoke `quality-gatekeeper` to validate output.
4. **Pruning**: Once finished, the skill is moved back to a "dormant" state to clear the context window.

---

## 🔎 REQUEST CLASSIFIER (STEP 2)

**Detailed Logic:** `.agent/core/classifier.md`
...
| **SLASH CMD** | /create, /orchestrate, /debug, /deep-solve | Command-specific flow | Variable |

---

## TIER 0: UNIVERSAL RULES (Always Active)

**Full Policy:** `.agent/core/rules.md`
...
### 🗺️ System Map Read
> 🔴 **MANDATORY**: Read `ARCHITECTURE.md` and `registry.json` at session start.
...
**Trigger**: When the user says "son kontrolleri yap", "final checks", or similar.
...
| `cloud-architect` | Cloud Infra & Serverless |
