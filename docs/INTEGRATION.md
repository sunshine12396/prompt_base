...
1. Antigravity automatically detects the `` folder in your project
2. It reads `GEMINI.md` at session start
...
1. Ensure `GEMINI.md` exists at your project root
...
Session Start
    ↓
Read GEMINI.md (rules, tiers, agents)
    ↓
On User Request → Consult registry.min.json
    ↓
Load relevant SKILL.md from skills/
    ↓
...
> 🔴 **MANDATORY**: You MUST read `GEMINI.md` at the start of every session.

This file (`.cursorrules`) is a pointer. The actual intelligence is in:

- **Rules**: `GEMINI.md`
- **Skills**: `registry.min.json`
```
...
- **Discovery**: Analyze intent and search `registry.min.json`.
- **Activation**: Load necessary `SKILL.md` files from `skills/`.
...
2. Ensure `` folder is included in indexing
...
```
your-project/
├──                     # ← Prompt Base Framework
│   ├── GEMINI.md              # Main configuration (read by both IDEs)
│   ├── ARCHITECTURE.md        # System documentation
│   ├── registry.min.json          # Skill discovery index
...
Installs everything inside `` to keep project root clean:
...
3. The AI should reference `GEMINI.md` and list the 19 agents
...
# Remove old framework (keep your customizations in custom/)
rm -rf agents skills/core skills/tech skills/process
...
> 💡 **Tip**: Custom skills in `skills/custom/` are preserved during updates.
...
- Ensure the file exists at `GEMINI.md`
...
- Verify `registry.min.json` exists and is valid JSON
- Check that skill paths in registry match actual file locations
- Run `python3 scripts/generate_registry.py .agent` to regenerate

---

> **Need help?** Open an issue on GitHub or check the [Architecture Guide](ARCHITECTURE.md).
